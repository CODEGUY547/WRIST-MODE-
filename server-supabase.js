...
function analyticsEvent(body = {}) { const eventType = String(body.eventType || ""); const sessionId = String(body.sessionId || "").replace(/[^a-zA-Z0-9-]/g, "").slice(0, 80); if (!analyticsEventTypes.has(eventType) || sessionId.length < 12) return null; const productId = Number(body.productId); return { session_id: sessionId, event_type: eventType, product_id: Number.isInteger(productId) && productId > 0 ? productId : null, page: String(body.page || "").slice(0, 40), created_at: now() }; }
...
app.post("/api/analytics/events", asyncRoute(async (req, res) => { const event = analyticsEvent(req.body); if (!event) return res.status(400).json({ error: "Invalid analytics event." }); const { error } = await supabase.from("analytics_events").insert(event); if (error) return res.status(202).json({ ok: false }); res.status(201).json({ ok: true }); }));
...
app.post("/api/orders", asyncRoute(async (req, res) => {
  ...
  const purchaseEvent = analyticsEvent({ ...req.body.analytics, eventType: "purchase" }); if (purchaseEvent) await supabase.from("analytics_events").insert(purchaseEvent);
...
app.get("/api/admin/analytics", requireAdmin, asyncRoute(async (req, res) => { const since = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString(); const [{ data: productRows, error: productError }, { data: orderRows, error: orderError }, { data: customRows, error: customError }, { data: eventRows, error: eventError }] = await Promise.all([supabase.from("products").select("*"), supabase.from("orders").select("*").order("created_at", { ascending: false }), supabase.from("custom_requests").select("*").order("created_at", { ascending: false }), supabase.from("analytics_events").select("session_id,event_type").gte("created_at", since)]); failIfError(productError); failIfError(orderError); failIfError(customError); const products = (productRows || []).map(productFromRow); const orders = (orderRows || []).map(orderFromRow); const totals = new Map(); for (const order of orders) for (const item of order.items) totals.set(item.name, (totals.get(item.name) || 0) + Number(item.quantity || 0)); const events = eventError ? [] : eventRows || []; const eventCount = (type) => events.filter((event) => event.event_type === type).length; const visitors = new Set(events.filter((event) => event.event_type === "visit").map((event) => event.session_id)).size; res.json({ totalProducts: products.length, totalOrders: orders.length, revenue: orders.reduce((sum, order) => sum + Number(order.total || 0), 0), customRequests: (customRows || []).length, lowStock: products.filter((item) => item.quantity > 0 && item.quantity <= 3).length, outOfStock: products.filter((item) => item.quantity <= 0).length, bestSelling: [...totals.entries()].sort((a, b) => b[1] - a[1]).slice(0, 5).map(([name, quantity]) => ({ name, quantity })), trackingActive: !eventError, funnel: { visitors, productViews: eventCount("product_view"), cartAdditions: eventCount("add_to_cart"), checkoutStarts: eventCount("checkout_started"), purchases: eventCount("purchase") } }); }));
